...
<div id="comments_wrapper" class="<?php print $classes; ?>"<?php print $attributes; ?>>
 <?php if($node->type === "article") :?>   
  <?php if ($content['comments']): ?>
	  ...
	  <p class="hint marginBottom bold">There are <span class="main-color"><?php print $node->comment_count ?></span> comments on this post</p>
  <?php endif; ?>
 ...
 <?php endif;?>
  ...
  <?php if($node->type === "product_display") :?>
       <?php print render($content['comments']); ?>
       <?php if ($content['comment_form']): ?>
        <h3 class="title-product-review"><?php print t('SUBMIT YOUR REVIEW');?></h3>
        <?php print render($content['comment_form']); ?>
       <?php endif; ?> 
  <?php endif;?>      
</div>
